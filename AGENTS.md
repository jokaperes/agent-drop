...
tool-neutral — there's nothing Claude-specific about how the app works. Keep it
accurate when behavior changes.

## What this project is

**Claude Drop** is a TanStack Start (React 19 + Nitro) + shadcn web app that moves
files between a phone browser and an *assistant that runs on the same machine*,
over a private network. "Claude" is just the name; any assistant or human with
...
scripts/
  ...
  cdrop                    CLI: copy a file into out/ + write caption sidecar
...
Reference deployment runs two systemd services — `claude-drop.service` (web app)
and `claude-drop-notify.service` (outbox watcher). After code changes:
